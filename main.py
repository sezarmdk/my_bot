...
# --- ULTRA-FAST STORY KUZATUVCHISI VA REAKSIYA BOSUVCHI ---
                ...
                try:
                    ...
                    for story in res.stories.stories:
                        ...
                        if s_id not in seen_list:
                            ...
                            # 2. Reaksiya (Like / Maxsus Emoji) bosish
                            try:
                                if emoji_id:
...
async def handle_commands(event):
    ...
    # 1. .story — KUZATUVGA QO'SHISH (KANAL YOKI FOYDALANUVCHI)
    if cmd == ".story":
        target = None
        emoji_id = None
        ...
        await event.edit(
            ...
            f"⚡️ Yangi story soniyalar ichida ko'rilib, like bosiladi."
        )
        await send_log(f"➕ **Kuzatuvga qo'shildi:** `{t_name}` (`{t_id}`)")

    # 2. .unstory — KUZATUVDAN O'CHIRISH
    elif cmd == ".unstory":
        target_id = None
        if event.is_reply:
            reply = await event.get_reply_message()
        ...
        if target_id in APP_DATA["story_targets"]:
            ...
            save_data(APP_DATA)
            await event.edit(f"🗑 **`{removed_name}` (`{target_id}`) kuzatuvdan olib tashlandi!**")
            await send_log(f"➖ **Kuzatuvdan chiqarildi:** `{removed_name}`")
        else:
            await event.edit("ℹ️ Bu manzil kuzatuv ro'yxatida yo'q.")

    # 3. .stat / .info — STATISTIKA VA RO'YXAT
        ...
        msg = (
            f"📊 **YAGONA PROFIL STORY BOT STATISTIKASI:**\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
...
async def handle_http_ping(request):
    return web.Response(text="Single-Profile Story Bot is Running")
...
async def main():
    ...
    await send_log("🚀 **Yagona profilli Story Watcher muvaffaqiyatli ishga tushdi!**")
